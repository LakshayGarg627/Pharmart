...
let newlaunchContainer=document.querySelector(".new-launches>.product-cards");
createProductList(newLaunchesList,newlaunchContainer);
...
function createProductBannerList(list,container){
    list.forEach(function(i){
        ...
        if(typeof i.discount=="number"){
            discount.innerText=i.discount+"% OFF";
            price.innerHTML="<span>₹"+i.mrp+"</span><br>"+"₹"+i.price;
        }else{
...
function createProductList(list,container){
    list.forEach(function(i){
        ...
        mrp.innerHTML="MRP <span> ₹"+ i.mrp+"</span>";
        let p=(i.mrp/100)*(100-i.discount);
        p=(Math.round(p*100)/100).toFixed(2);
        price.innerHTML="₹"+p+"<span>"+i.discount+"% OFF";
